...
void ConvertMACAddress(const uint8_t src[6], uint16_t dst[3]) {
    for (int i = 0; i < 3; i++) {
        // ���ֽ���ǰ�����ֽ��ں� (�����)
        dst[i] = (src[i*2] << 8) | src[i*2 + 1];
    }
}
...
void ec_setupheader(void* p) {
  ec_etherheadert* bp;
	...
	ConvertMACAddress(ETH_MAC_ADDR, priMAC);
...
int ecx_outframe(ecx_portt* port, int idx, int stacknumber) {
  ...
  if (len>0) {
	...
	//DEBUG_PRINT("Sent idx=%d, state=EC_BUF_TX\n", idx);
...
int ecx_outframe_red(ecx_portt* port, int idx) {
  ...
  rval = ecx_outframe(port, idx, 0);
	//DEBUG_PRINT("Frame received: idxf=%d\n", idx);
 ...
 * @param[in] port        = port context struct
 * @param[in] stacknumber = 0=primary 1=secondary stack
 * @return >0 if frame is available and read, -1 if error
 */
static int ecx_recvpkt(ecx_portt* port, int stacknumber) {
    ...
    // 调用你写的 HAL 接收函数，将数据放入 tempbuf 中
    bytesrx = ecx_nic_receive(port, *stack->tempbuf, sizeof(ec_bufT));
		//printf("RecvPkt: len=%d\n", bytesrx);
    ...
    if (ecx_recvpkt(port, stacknumber)) {
      ...
      if (pEthHeader->etype == oshw_htons(ETH_P_ECAT)) {
        ...
        idxf = pEcHeader->index;
				//DEBUG_PRINT("Received idxf=%d, state=%d (EC_BUF_TX=1?)\n", idxf, (*stack->rxbufstat)[idxf]);
        ...
        if (idxf == idx) {
          ...
          if (idxf < EC_MAXBUF && (*stack->rxbufstat)[idxf] == EC_BUF_TX) {
            ...
            /* strange things happend */
						DEBUG_PRINT("strange things: idxf=%d, EC_MAXBUF=%d, rxbufstat[idxf]=%d\n", 
            idxf, EC_MAXBUF, (*stack->rxbufstat)[idxf]);
...
static int ecx_waitinframe_red(ecx_portt* port, int idx, osal_timert timer) {
  ...
  do {
    ...
    if (wkc <= EC_NOFRAME) {
        ...
        // uint32_t timeout =
        //     diffTime.sec * 1000 + diffTime.usec / 1000;  // ��λ: ms
